...
//3.2　回転移動行列の作成
	...
	float theta = ((float)(id_v + h_low)/180.0)*M_PI;
	float theta_gyaku = ((float)(id_v + h_low + 180.0)/180.0)*M_PI;
	transform_2.rotate (Eigen::AngleAxisf (-theta, Eigen::Vector3f::UnitZ()));
	transform_2_gyaku.rotate (Eigen::AngleAxisf (-theta_gyaku, Eigen::Vector3f::UnitZ()));

	cout<< "theta=" << theta <<endl;
	cout<< "theta_gyaku=" << theta_gyaku <<endl;

	cout << "\nMethod #2: using an Affine3f\n";
	std::cout << transform_2.matrix() << std::endl;
	cout << "\nMethod #2: using an Affine3f_gyaku\n";
	std::cout << transform_2_gyaku.matrix() << std::endl;
...
//3.3 点群の移動	(ここでも２つつくる)：：初期位置用点群
	...
	pcl::PointCloud<pcl::PointXYZRGBNormal>::Ptr transformed_cloud_gyaku (new pcl::PointCloud<pcl::PointXYZRGBNormal> ());
  	...
  	pcl::transformPointCloud (*out_cloud, *transformed_cloud_gyaku, transform_2_gyaku);
